<div x-show="step === 3" x-transition>
    <div class="sharp-card">
        <h2 class="text-2xl font-black mb-8 border-b pb-4">03. الخبرات العملية</h2>
        <template x-for="(exp, index) in experiences" :key="exp.id">
            <div class="mb-10 p-6 border border-gray-200 bg-gray-50 relative">
                <button type="button" @click="experiences.splice(index, 1)" class="absolute top-0 left-0 bg-red-600 text-white px-3 py-1 text-xs font-bold">حذف</button>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
                    <div>
                        <label class="sharp-label">الشركة</label>
                    ...
                    <div>
                        ...
                        <div class="mt-3 flex items-center">
                            ...
                            <label class="text-sm font-bold text-gray-700">ما زلت أعمل هنا</label>
                        </div>
                    </div>
                    <div class="col-span-full">
                        <div class="flex justify-between items-center mb-2">
                            <label class="sharp-label mb-0">الوصف والمهام</label>
                            <button type="button" class="sharp-btn-ai" @click="generateExperienceAI(index)" :disabled="aiCredits <= 0">✨ توليد بالذكاء الاصطناعي</button>
                        </div>
        ...
        <button type="button" @click="addExperience()" class="sharp-btn-secondary">+ إضافة خبرة جديدة</button>
    </div>
</div>