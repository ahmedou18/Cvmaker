...
<!DOCTYPE html>
<html lang="{{ $resumeLanguage }}" dir="{{ $resumeLanguage == 'ar' ? 'rtl' : 'ltr' }}">
<head>
    <meta charset="UTF-8">
...
<div class="print-container max-w-5xl mx-auto bg-white shadow-2xl rounded-lg overflow-hidden flex flex-col md:flex-row min-h-[1122px]">
    ...
    {{-- Sidebar --}}
    <div class="w-full md:w-[35%] bg-primary text-white p-8 print:p-6">

        @if($photoUrl)
        <div class="flex justify-center mb-6">
            <img src="{{ $photoUrl }}" class="w-40 h-40 rounded-sm object-cover border-4 border-white shadow-md">
        </div>
        @endif

        <div class="mb-6 text-center md:text-start">
            <h1 class="text-3xl font-extrabold">{{ $profile->full_name }}</h1>
            <p class="text-primary-100 text-lg opacity-90">{{ $profile->job_title }}</p>
        </div>

        {{-- الاتصالات --}}
        <div class="mb-6 space-y-3 text-sm">
            @if($profile->email)<div class="flex items-center gap-2"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg><span class="break-all">{{ $profile->email }}</span></div>@endif
            @if($profile->phone)<div class="flex items-center gap-2"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg><span dir="ltr">{{ $profile->phone }}</span></div>@endif
            @if($profile->address)<div class="flex items-start gap-2"><svg class="w-5 h-5 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg><span>{{ $profile->address }}</span></div>@endif
        </div>

        {{-- الأقسام الإضافية (في الجانب) --}}
        @if(!empty($extra))
    ...
    <div class="w-full md:w-[65%] p-8 space-y-8">
        ...
        @if($profile->summary)
        <section>
            <div class="bg-primary-light px-3 py-1.5 mb-4 border-r-4 border-primary">
        ...
        <section>
            ...
            <div class="space-y-6">
                ...
                <div class="relative pr-4 border-r-2 border-gray-200">
                    ...
                    @if($exp->description)<p class="text-gray-700 text-sm mt-1">{{ $exp->description }}</p>@endif
                </div>
                @endforeach
            </div>
        </section>
