...
<body class="text-slate-800 antialiased overflow-x-hidden">
    ...
    {{-- ========== شريط التنقل (النسخة المحسّنة) ========== --}}
    <nav class="glass-nav fixed top-0 w-full z-50 transition-all" x-data="{ mobileMenuOpen: false, scrolled: false }" x-init="window.addEventListener('scroll', () => { scrolled = window.scrollY > 20 })" :class="scrolled ? 'shadow-sm' : ''">
    ...
    <main>
        ...
        <section class="relative pt-32 md:pt-48 pb-20 md:pb-32 px-6 overflow-hidden">
            ...
            <div class="max-w-5xl mx-auto text-center z-10 relative">
                ...
                <p class="text-lg md:text-xl text-slate-500 mb-10 max-w-2xl mx-auto leading-relaxed" data-aos="fade-up" data-aos-delay="100">{{ __('messages.hero_description') ?? 'منصة ذكية تساعدك على تصميم سيرة ذاتية تبرز مهاراتك وتزيد من فرص قبولك في وظيفة أحلامك.' }}</p>
                        ...
                        <div class="swiper-slide w-[280px] sm:w-[320px]">
                            <div class="group relative rounded-2xl bg-slate-50 p-3 border border-slate-100 transition-all duration-300 hover:shadow-2xl hover:shadow-indigo-500/10 hover:-translate-y-2">
                                <div class="aspect-[1/1.4] bg-slate-200 rounded-xl overflow-hidden relative">
                                    @if($template->thumbnail)<img src="{{ asset($template->thumbnail) }}" alt="{{ $template->name }}" class="w-full h-full object-cover">@else<div class="w-full h-full bg-gradient-to-br from-slate-200 to-slate-300 flex items-center justify-center text-slate-500 font-bold">معاينة القالب</div>@endif
                                    <div class="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-[2px]"><a href="{{ route('templates.choose') }}" class="bg-white text-slate-900 px-6 py-2.5 rounded-full font-bold text-sm hover:scale-105 transition-transform z-10">استخدام القالب</a></div>
                                </div>
                                <div class="pt-4 pb-2 px-2 flex justify-between items-center"><h3 class="font-bold text-slate-800">{{ $template->name }}</h3><span class="text-xs font-bold px-2.5 py-1 bg-slate-200/50 text-slate-600 rounded-md">{{ isset($template->is_premium) && $template->is_premium ? '💎 احترافي' : '✅ مجاني' }}</span></div>
                            </div>
                        </div>
                        @empty
    ...
    <footer class="py-8 border-t border-slate-100 bg-white">
        <div class="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-4 text-sm text-slate-500">
            ...
            <div class="flex gap-6"><a href="#" class="hover:text-indigo-600">الشروط</a><a href="#" class="hover:text-indigo-600">الخصوصية</a><a href="#" class="hover:text-indigo-600">تواصل معنا</a></div>
        </div>
    </footer>
