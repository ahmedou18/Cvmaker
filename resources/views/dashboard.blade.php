        ...
        <div class="flex justify-between items-center" dir="rtl">
            ...
            @can('create', App\Models\Resume::class)
                <a href="{{ route('templates.choose') }}"
                   class="bg-indigo-600 text-white px-5 py-2 rounded-full text-sm font-bold shadow hover:bg-indigo-700 transition">
                    + {{ __('messages.create_new_resume') }}
                </a>
            @else
                <button onclick="openPlansModal()"
                        class="bg-amber-500 text-white px-5 py-2 rounded-full text-sm font-bold shadow hover:bg-amber-600 transition">
        ...
        <div class="max-w-6xl mx-auto px-6 space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="bg-white p-5 rounded-2xl border border-slate-100">
                    ...
                    <h3 class="text-lg font-bold text-indigo-600">
                        {{ $user->plan?->name ?? __('messages.none') }}
                    </h3>
                </div>
                <div class="bg-white p-5 rounded-2xl border border-slate-100">
            ...
            <div class="bg-white p-5 rounded-2xl border border-slate-100">
                <div class="flex justify-between text-sm mb-2">
                    <span>{{ __('messages.subscription_usage') }}</span>
                    <span>{{ number_format($usage, 0) }}%</span>
                </div>
            ...
            {{-- Quick Actions (بدون دعم) --}}
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <a href="{{ route('templates.choose') }}"
                   class="bg-white p-5 rounded-2xl border border-slate-100 hover:shadow transition">
                ...
                @else
                    <button onclick="openPlansModal()"
                            class="bg-white p-5 rounded-2xl border border-slate-100 text-right opacity-80">
            ...
            <div>
                    ...
                    @foreach($resumes as $resume)
                        <div class="bg-white p-5 rounded-2xl border border-slate-100 hover:shadow transition">
                            <div class="flex justify-between items-center mb-2">
                                ...
                                <span class="text-xs px-2 py-0.5 rounded-full {{ $resume->is_published ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700' }}">
                            ...
                            <div class="flex justify-between text-sm">
                                <a href="{{ route('resume.show', $resume->uuid) }}"
                ...
                @else
                    <div class="bg-white p-8 rounded-2xl border border-dashed border-slate-200 text-center">
                        ...
                        <a href="{{ route('templates.choose') }}" class="mt-3 inline-block text-indigo-600 text-sm font-medium hover:underline">
                            {{ __('messages.create_new_resume') }}
                        </a>
                    </div>
                @endif
